...
def setDfCpuLoad2g3gEms(ems):
	...
	for file in list_file :
		result = re.search('.+ARAPCPUMEMMO'+'.+',file)
		...
		if result :
			...
			df_res['tech'] = '2G'
			df_res['OSS'] = ems
			df_res['max_ratio_cpu'] = df_res['C380360003']
			df_res['mean_ratio_cpu'] = df_res['C380360004']
			df_result = df_res[
				[
					'COLLECTTIME'
					,'GRANULARITY'
...
def setDfCpuLoad2g3gUme(ume):
	...
	filedir = config_data['SRC_UME']['CpuLoad2G3G'][0]['src_dir'] + os.sep + dirdate
	...
	if ume == "UME_SUL" or ume == "UME_KAL" or ume == "UME_PUMA":
		...
		for file in os.listdir(extract_to_dir) :
			...
			# set tech
			df_res['tech'] = np.where(df_res['SubnetWork Name'].str.contains('BSC'),'2G','3G')
			df_res['OSS'] = ume
			# set COLLECTTIME
			...
			df_result = df_res[
				[
					...
					,'OSS'
					,'tech'
					,'Max ratio of CPU usage(%)'
					,'Mean ratio of CPU usage(%)'
				]
			]
			df_result.rename(columns={
				'SubnetWork ID' : 'SUBNETWORK'
				,'Max ratio of CPU usage(%)' : 'max_ratio_cpu'
				,'Mean ratio of CPU usage(%)' : 'mean_ratio_cpu'
			},inplace=True)

	df_result['max_ratio_cpu'] = df_result['max_ratio_cpu'].str.rstrip('%').astype('float')/100
	df_result['mean_ratio_cpu'] = df_result['mean_ratio_cpu'].str.rstrip('%').astype('float')/100
	return df_result

def setDfCpuLoad4gEms(ems,band):
	curdir = setCurDir()
	if ems == "EMS5" :
		curdate = (datetime.today() - timedelta(minutes=0)).strftime('%d%b%Y')
	else :
	...
	for file in list_file :
		result = re.search('.+ENBSYSRES'+'.+',file)
		if result :
			# print(result.group(0))
			filename = result.group(0)
			df_res = pd.read_csv(file_dir+os.sep+filename)
			...
			df_res['OSS'] = ems
			df_res['GRANULARITY'] = 900
			df_res['max_ratio_cpu'] = df_res['C373940400']
			df_res['mean_ratio_cpu'] = df_res['C373940401']
			df_result = df_res[
				[
					'COLLECTTIME'
					,'GRANULARITY'
...
def setDfCpuLoad4gUme(ume):
	...
	filedir = config_data['SRC_UME']['CpuLoad4G'][0]['src_dir'] + os.sep + dirdate
		...
		for file in os.listdir(extract_to_dir) :
			...
			if ume == "UME_SUL" :
				...
				df_result.rename(columns={
					...
					,'eNodeBId' : 'SITEID'
					,'Peak CPU Utilization Rate of ENB CC Board(%)' : 'max_ratio_cpu'
					,'Average CPU Utilization Rate of ENB CC Board(%)' : 'mean_ratio_cpu'
				},inplace=True)
			elif ume == "UME_PUMA" :
				df_result = df_res[
					[
...
def joining_df_4g_ems(oss):
	df_fdd = setDfCpuLoad4gEms(oss,"FDD")
	df_tdd = setDfCpuLoad4gEms(oss,"TDD")
	df_result = pd.concat([df_fdd,df_tdd])

	return df_result

def parsing_cpu_load_2g3g():
	# No More EMS #
	# df_ems5 = setDfCpuLoad2g3gEms("EMS5")
	# df_ems6 = setDfCpuLoad2g3gEms("EMS6")
	# df_ems7 = setDfCpuLoad2g3gEms("EMS7")
	df_ume_sul = setDfCpuLoad2g3gUme("UME_SUL")
	df_ume_puma = setDfCpuLoad2g3gUme("UME_PUMA")
	df_ume_kal = setDfCpuLoad2g3gUme("UME_KAL")
...
def counting_cpu_load_2g3g():
	curdir = setCurDir()
	# print(curdir)
	df_cpload = parsing_cpu_load_2g3g()
	df_cpload['controllerPrimKey'] = df_cpload['SUBNETWORK'].astype(str)
	df_controller = pd.read_csv(curdir+os.sep+'controller_data.csv')
	df_controller['controllerPrimKey'] = df_controller['subnetwork_id'].astype(str)
	# return df_data_poi
	df_merge = df_cpload.merge(df_controller,on='controllerPrimKey',how='inner')
	df_pivot = np.round(pd.pivot_table(df_merge,values=['max_ratio_cpu','mean_ratio_cpu'],index=['subnetwork_id'
		,'subnetwork_name','oss','regional','tech_y'],aggfunc=np.max),2)
...
def counting_cpu_load_4g():
	...
	df_cpload['sitePrimKey'] = df_cpload['CONTROLLERID'].astype(str)+df_cpload['SITEID'].astype(str)
	...
	df_pivot = np.round(pd.pivot_table(df_merge,values=['max_ratio_cpu','mean_ratio_cpu'],index=['SITE_NAME','REGION'],aggfunc=np.max),2)
	df_result = df_pivot.reset_index()
	return df_result
